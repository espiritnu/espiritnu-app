...
const BreathingExercise = ({
  ...
  useEffect(() => {
    ...
    const runCycle = () => {
      ...
      setInstruction(sequence[currentStep].text);
      cycleRef.current = setTimeout(runCycle, sequence[currentStep].duration);
    };
  ...
  if (!show) return null;
  return (
    <div className="fixed inset-0 bg-[var(--color-bg)] flex flex-col items-center justify-center animate-fadeIn z-50">
      <button
        onClick={onClose}
        className="absolute top-6 right-6 text-2xl text-[var(--color-text-subtle)] hover:text-[var(--color-text)]"
      >
        ✕
      </button>
      <div className="relative flex items-center justify-center w-64 h-64">
        <div
          className="w-full h-full rounded-full border-2 border-[var(--color-glow)]"
          style={{ animation: `breatheGlowAnimation 14s ease-in-out infinite` }}
        />
        <p className="absolute text-2xl font-lora font-bold text-[var(--color-text)]">
          {instruction}
        </p>
      </div>
      <p className="absolute bottom-10 text-lg text-[var(--color-text-subtle)]">
        {countdown}
      </p>
    </div>
  );
      ...
      case "WhisperReveal":
        return (
          <WhisperRevealScreen
            ...
            onStartBreathing={() => setIsBreathingExerciseOpen(true)}
          />
        );
      case "Journaling":
        return (
